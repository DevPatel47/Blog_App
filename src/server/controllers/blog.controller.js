...
const createBlog = asyncHandler(async (req, res) => {
    ...
    if ( title.length < 5 || title.length > 70 ) {
        return res
            .status(400)
            .json(new ApiError(400, "Blog's title must be between 5 to 70 characters"));
    };
    ...
    if ( content.length < 20 || content.length > 5000 ) {
        return res
            .status(400)
            .json(new ApiError(400, "Blog's content must be between 20 to 5000 characters"))
    }
...
const likeBlog = asyncHandler(async (req, res) => {
    ...
    if (!blog) {
        return res
            .status(404)
            .json(new ApiError(400, "Blog not found"));
    }
    ...
    if (alreadyLiked) {
        return res
            .status(400)
            .json(new ApiError(400, "Blog is already liked"));
    }
...
const editBlog = asyncHandler(async (req, res) => {
    ...
    if (!editedBlog) {
        return res
            .status(500)
            .json(
                new ApiError(500, "Something went wrong while updating the blog")
            );
    }
